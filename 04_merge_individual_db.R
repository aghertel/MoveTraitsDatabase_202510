...
metadata <- readRDS(paste0(
  pathTOfolder2,
  "/referenceTableStudies_ALL_excludedColumn_excludedStudies.rds"
))

# note to self - add "individual_local_identifier"
metadata <-
  metadata |>
  mutate(
    individual_id = sapply(str_split(fileName, "_|\\."), function(x) x[2])
  ) |>
  dplyr::select(
    MBid,
    individual_id,
...
db.movebank.3[
  db.movebank.3$species %in% c("Pekania pennanti"),
  "species"
] <- "Martes pennanti"

unique(db.movebank.3$species)
...
table(db.movebank.4[db.movebank.4$include == "no", c("species")])
...
db.tucker <- readRDS(
  "DATA/MoveTraitsData/Tucker/tucker_individual.sum_20260521.rds"
)
...
db.tucker <- db.tucker |>
  ...
  filter(
    !(species == "Cervus canadensis" &
      contact_person_name == "Mark Hebblewhite")
  )
...
individual.db <- individual.db |>
  mutate(
    species = fct_recode(
      species,
      "Ovis canadensis" = "Ovis canadensis californiana"
    )
  ) |>
  mutate(
    species = fct_recode(
      species,
      "Ovis canadensis" = "Ovis canadensis canadensis"
    )
  ) |>
  mutate(
    species = fct_recode(species, "Ovis canadensis" = "Ovis canadensis nelsoni")
  ) |>
  mutate(
    species = fct_recode(species, "Loxodonta africana" = "Elephantidae")
  ) |>
