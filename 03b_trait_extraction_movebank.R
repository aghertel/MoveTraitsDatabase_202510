...
saveRDS(MoveTrait.repeats, file=paste0(pthtrait,indPth))
    ...
    message("Skipping ", indPth, " — ", conditionMessage(e))
    failed_files <<- c(failed_files, indPth)
    NULL
  })
...
if (length(failed_files) > 0) {
  saveRDS(failed_files,
          file = paste0(pathTOfolder, "failed_movebank_files_",
                        format(Sys.time(), "%Y%m%d"), ".rds"))
  message(length(failed_files), " file(s) failed — see failed_files RDS in ", pathTOfolder)
} else {
