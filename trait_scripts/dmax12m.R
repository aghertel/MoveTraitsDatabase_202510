...
if(is.null(dmax12m)) NULL else {
  ...
  cell_info_100.a <- dgGEO_to_SEQNUM(dggs_100, dmax12m$lon_start, dmax12m$lat_start)
  cell_info_100.b <- dgGEO_to_SEQNUM(dggs_100, dmax12m$lon_end, dmax12m$lat_end)
  dmax12m$grid.id.100km <- cell_info_100.a$seqnum
  dmax12m$grid.id.100km <- paste(dmax12m$grid.id.100km,cell_info_100.b$seqnum,sep=";")
  
  # Spatial annotation 10km
  cell_info_10.a <- dgGEO_to_SEQNUM(dggs_10, dmax12m$lon_start, dmax12m$lat_start)
  cell_info_10.b <- dgGEO_to_SEQNUM(dggs_10, dmax12m$lon_end, dmax12m$lat_end)
  ...
  cell_info_1.a <- dgGEO_to_SEQNUM(dggs_1, dmax12m$lon_start, dmax12m$lat_start)
  cell_info_1.b <- dgGEO_to_SEQNUM(dggs_1, dmax12m$lon_end, dmax12m$lat_end)
...
f_sum.ind.dmax12m<-function(x)
{
  ...
  if (is.null(x)) {
    ...
    dats <- data.frame(individual_id = NA_character_,n.max12m.years = NA,
